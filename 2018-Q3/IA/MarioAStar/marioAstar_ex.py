...
def melhor_filho(tree):
    ...
    if len([x for x, mf in f if mf is None]) == len(tree.filhos):
...
def expande(tree, env, mostrar):
    ...
    if folha(tree):
        ...
        while raiz.pai is not None:
            ...
            acao = [acao for acao, filho in raiz.filhos.items() if filho == neto][0]
...
def atingiuObj(tree):
    ...
    for k, filho in tree.filhos.items():
        obj, acoes = atingiuObj(filho)
        if obj:
            return True, acoes + [moves[k]]
